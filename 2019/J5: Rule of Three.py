...
class node:
    ...
    def child_values(self, rules, steps):
        ...
        for i in out_values:
            self.children.append(node(i[0], parent = self, ruleUsed = i[1], indexChanged = i[2]+1))
    ...
    def backtrack(self, leaf):
        ...
        for i in range(int(steps)):
            output.append([rulesList.index(current.ruleUsed)+1, current.indexChanged, current.val])
