...
int main() {
    ...
    uint needleLength =
        needle.size(); // Get needle length now so we don't call len all the time
    ...
    map<char, uint>
        prevLetters; // Same as needleLetters, but for each part of the haystack
    ...
    for (uint index = 0; index < haystack.size() - needleLength + 1; index++) {
        ...
        /**
         * If no previous letter is found, perform the same loop as performed
         * above. Otherwise, we can use the previous data. We can subtract 1 from
         * the previous window's first letter, and add 1 to the current windows
         * last letter. This way, we don't needlessly loop through needleSlice
         * again.
         */
        if (index == 0) {
        ...
        /**
         * We need to use a hashing algorithm, because after test 114, the
         * strings become too large, and we get a memory error in the grader.
         */
